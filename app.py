    ...
    with c2:
        st.markdown(f"""
            <div class="login-card">
                <h2 style='text-align: center; color: {C_AZUL}; margin-top: 0;'>Bem-vindo!</h2>
                <p style='text-align: center; color: #666; font-size: 14px;'>Acesse o sistema do Instituto Mãe Lalu</p>
            </div>
        """, unsafe_allow_html=True)
        with st.form("login"):
...
if menu == "📝 Matrículas":
    ...
    f_tn = c1.selectbox("Turno ", ["Todos", "A", "B"])
    f_cm = c2.selectbox("Comunidade ", ["Todas"] + sorted(list(df["COMUNIDADE"].unique())))
    f_sp = c3.checkbox("Apenas sem padrinho")
    df_f = df.copy()
    if f_tn != "Todos": df_f = df_f[df_f["TURNO"].astype(str).str.contains(f_tn)]
    if f_cm != "Todas": df_f = df_f[df_f["COMUNIDADE"] == f_cm]
    ...
    with st.form("aval"):
        ...
        if st.form_submit_button("Salvar Avaliação"):
            ...
            novo = pd.DataFrame([[al, tr] + [float(v) for v in notas.values()]], columns=df_av.columns)
            pd.concat([df_av, novo], ignore_index=True).to_csv(AVAL_FILE, index=False)
    ...
    if not afilhas.empty:
        al_s = st.selectbox("Selecione seu afilhado:", afilhas["ALUNO"].unique())
        ...
        else: st.info("Avaliações ainda não disponíveis.")
    ...
    if not df_av.empty:
        ...
        df_al = df_av[df_av["Aluno"] == al_s]
        tri = st.selectbox("Ver Período:", df_al["Trimestre"].unique(), key="tri_int")
        row = df_al[df_al["Trimestre"] == tri].iloc[0]
    ...
    with st.form("cad"):
        ...
        if st.form_submit_button("Finalizar Cadastro"):
            ...
            st.success(f"O aluno {n.upper()} foi cadastrado!")
