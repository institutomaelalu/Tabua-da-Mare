...
if menu == "📝 Controle de Matrícula e Apadrinhamento":
    ...
    # Cores Identidade Visual (Azul, Verde, Laranja)
    estilo_design_anos = {
        2025: ("#E8F4F8", "#2E86C1"), 
        2026: ("#E8F8F5", "#28B463"), 
        2027: ("#FEF5E7", "#E67E22")
    ...
    for i, ano in enumerate(st.session_state.lista_anos_te):
        ...
        # Injeção de CSS para forçar o formato Pílula Arredondada
        st.markdown(f"""
            <style>
            div[data-testid="column"]:nth-of-type({i+1}) button {{
                ...
                border-radius: 50px !important; /* Aqui garante que fica redondo */
                font-weight: bold !important;
    ...
    # --- SELEÇÃO DE SALA (PADRÃO DAS OUTRAS ABAS) ---
    if salas_te_disponiveis:
        # Agora usando a função padrão que você usa nas outras abas
        render_botoes_salas("btn_te_sala", "sel_te_sala", salas_permitidas=salas_te_disponiveis)
        
        # Recupera a sala selecionada pelos botões pílula
        sala_v = st.session_state.get("sel_te_sala", salas_te_disponiveis[0])
        
        alunos_filtro = [n for n, s in dict_alunos_te.items() if s == sala_v]
        
        st.write(f"Alunos da Sala **{sala_v}**:")
        ...
        for i, n_t in enumerate(NIVEIS_ALF):
            ...
            ht += f'<div class="box-te" style="background-color:{bg}; color:{txt}; border:{borda}; opacity:{"1" if is_active else "0.3"}">{n_t.split(". ")[1]}</div>'
