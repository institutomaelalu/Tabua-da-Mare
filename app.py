...
def safe_read(worksheet_name):
    try:
        ...
        # Se falhar a conexão, retorna pelo menos o que tem no local
        try:
            return pd.read_csv(ALUNOS_FILE).fillna("")
        except:
            return pd.DataFrame(columns=["ALUNO", "TURMA", "TURNO", "IDADE", "COMUNIDADE"])

def render_styled_table(df, context_color=None):
    if df.empty:
        st.info("Aguardando carregamento ou sem dados para os filtros selecionados.")
        return
...
if menu == "👤 1. Novo Cadastro":
    ...
    # Filtros fora do IF para não sumirem
    f1, f2 = st.columns(2)
    with f1: f_nome = st.text_input("Buscar por Nome")
    with f2:
        opcoes_turno = ["Todos"] + sorted([str(x) for x in df["TURNO"].unique() if x and str(x).strip() != ""]) if not df.empty else ["Todos"]
        f_turno = st.selectbox("Filtrar por Turno", opcoes_turno)
    ...
    check_vazios = st.checkbox("Somente sem Padrinho")
    
    if not df.empty:
        df_f = df.copy()
        if check_vazios and "PADRINHO/MADRINHA" in df_f.columns:
            df_f = df_f[df_f["PADRINHO/MADRINHA"].astype(str).str.strip().isin(["", "nan", "None"])]
        
        exibir = [c for c in ["ALUNO", "TURMA", "IDADE", "COMUNIDADE", "PADRINHO/MADRINHA"] if c in df_f.columns]
        render_styled_table(df_f[exibir], context_color=sala)

elif menu == "📊 4. Avaliação - Tábua da Maré":
    st.header("📊 Registro de Notas")
    df_g = safe_read("GERAL")
    
    # Garantia de que a lista de alunos não quebre se a planilha falhar
    lista_alunos = []
    ...
    if lista_alunos:
        ...
        st.warning("Carregando lista de alunos...")
    ...
    if os.path.exists(AVAL_FILE):
        ...
        if not df_av.empty:
            ...
            # Pega a linha da avaliação e converte para numérico
            row = df_al[df_al["Trimestre"] == tri_s].iloc[0]
            valores = [float(row[c]) for c in CATEGORIAS]
            
            fig = go.Figure(go.Scatter(x=CATEGORIAS, y=valores, mode='lines+markers', fill='tozeroy', line=dict(color=COR_AZUL_INST, width=3)))
            fig.update_layout(yaxis=dict(range=[0, 5.5], tickvals=[1,2,3,4,5]), margin=dict(l=20, r=20, t=20, b=20))
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("Nenhuma avaliação registrada ainda. Vá em 'Lançar Avaliação'.")
    else:
        st.error("Arquivo de avaliações não encontrado.")
