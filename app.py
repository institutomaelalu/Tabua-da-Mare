...
# --- ABA: TURNO ESTENDIDO ---
    ...
    try:
        # Chamada simplificada usando a conexão configurada
        df_h = conn.read(worksheet="TURNO_ESTENDIDO").fillna("")
        # Padroniza nomes das colunas para evitar erros de case (Aluno, Sala, Diagnóstico...)
    ...
    # Converte explicitamente para int para evitar o erro StreamlitMixedNumericTypesError
    ...
    if "lista_anos_te" not in st.session_state:
        st.session_state.lista_anos_te = [int(a) for a in (anos_na_planilha if anos_na_planilha else [2025, 2026])]
    
    if "ano_registro_te" not in st.session_state: 
        st.session_state.ano_registro_te = int(st.session_state.lista_anos_te[-1])
    ...
    for i, ano in enumerate(st.session_state.lista_anos_te):
        ...
        if cols_anos_all[i].button(f"📅 {ano}", key=f"btn_reg_ano_{ano}", use_container_width=True):
            st.session_state.ano_registro_te = int(ano)
            st.rerun()
        
        # Estilo Pílula Arredondada
        st.markdown(f"<style>div[data-testid='stHorizontalBlock'] div:nth-child({i+1}) button {{ background-color: {cor_btn} !important; color: {txt_cor} !important; border: {'2px solid black' if is_active else '1px solid #ccc'} !important; border-radius: 50px !important; font-weight: bold !important; }}</style>", unsafe_allow_html=True)

    with cols_anos_all[len(st.session_state.lista_anos_te)].popover("➕"):
        # Correção do erro Mixed Types: garantindo que todos os valores sejam int
        val_default = int(st.session_state.lista_anos_te[-1] + 1)
        novo_ano_input = st.number_input("Digite o novo ano:", min_value=2024, max_value=2100, value=val_default, step=1)
        
        if st.button("Confirmar Novo Ano"):
            novo_int = int(novo_ano_input)
            if novo_int not in st.session_state.lista_anos_te:
                st.session_state.lista_anos_te.append(novo_int)
                st.session_state.lista_anos_te.sort()
                st.session_state.ano_registro_te = novo_int
                st.rerun()

    st.write(f"Registrando para o ano letivo: **{st.session_state.ano_registro_te}**")
    st.markdown("---")

    # --- SELEÇÃO DE ALUNO (MAPEADO DA ABA TURNO_ESTENDIDO) ---
    # Busca o dicionário de alunos/salas configurado no início do app
    dict_te = st.session_state.get("alunos_te_dict", {})
    salas_te = sorted(list(set(dict_te.values())))
    
    if salas_te:
        if "sel_te" not in st.session_state or st.session_state.sel_te not in salas_te: 
            st.session_state.sel_te = salas_te[0]
        
        # Botões coloridos das salas
        render_botoes_salas("btn_te", "sel_te", salas_permitidas=salas_te)
        
        al_te = [n for n, s in dict_te.items() if s == st.session_state.sel_te]
        al = st.selectbox("Aluno:", sorted(al_te))
        
        # Puxa o histórico do aluno para a trilha
        ...
        for i, n_t in enumerate(NIVEIS_ALF):
            ...
            opacidade = "1.0" if is_current else "0.5"
        ...
        try:
            ultimo_diag = diag["Diagnóstico"] if (diag is not None and diag["Diagnóstico"] in NIVEIS_ALF) else NIVEIS_ALF[0]
            idx_inicial = NIVEIS_ALF.index(ultimo_diag)
        except:
            idx_inicial = 0
        ...
        with st.form("f_te_nuvem"):
            ...
            for i, ev in enumerate(evidencias_atuais):
                if e_cols[i % 3].checkbox(ev, key=f"chk_te_{nV}_{i}"):
                    s_ev.append(ev)
        ...
        st.warning("Nenhuma configuração de alunos encontrada para o Turno Estendido. Verifique a planilha.")
