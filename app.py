...
# 2. Inicialização do Banco de Dados Local (Cria os arquivos se não existirem)
    ...
    if df_alunos.empty:
        ...
        with st.form("notas", clear_on_submit=True):
            ...
            if st.form_submit_button("Salvar Avaliação"):
                ...
                # Remove duplicatas para o mesmo aluno e trimestre
...
# --- PAINEL DE EVOLUÇÃO (DASHBOARD) ---
    ...
    if df_av.empty:
        st.info("Aguardando lançamentos de avaliações.")
    else:
        # Filtros encadeados em colunas
        c1, c2, c3 = st.columns(3)
        with c1:
            turno_sel = st.selectbox("1. Filtrar Turno", ["Matutino", "Vespertino"])
        
        # Alunos do turno que possuem notas
        alunos_turno = df_alunos[df_alunos["Turno"] == turno_sel]["Nome"].unique()
        avaliados = df_av[df_av["Aluno"].isin(alunos_turno)]["Aluno"].unique()
        
        if len(avaliados) == 0:
            st.warning("Sem avaliações para este turno.")
        else:
            with c2:
                aluno_sel = st.selectbox("2. Selecionar Aluno", sorted(avaliados))
            
            with c3:
                trims_disp = df_av[df_av["Aluno"] == aluno_sel]["Trimestre"].unique()
                trim_sel = st.selectbox("3. Escolha o Trimestre", trims_disp)
            ...
            # --- GRÁFICO DE MARÉ ULTRA SUAVE ---
            st.subheader(f"🌊 Tábua da Maré: {aluno_sel} ({trim_sel})")
            
            # Interpolação para suavizar a curva entre os pontos
