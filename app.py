...
# --- ABA: TURNO ESTENDIDO ---
    ...
    try:
        # Chamada usando a conexão padrão do app
        df_h = conn.read(worksheet="TURNO_ESTENDIDO").fillna("")
        # Padronização de nomes de colunas (Título) para evitar erros de busca
        df_h.columns = [str(c).strip().title() for c in df_h.columns]
    except Exception as e:
        st.error(f"Erro ao conectar com a planilha: {e}")
        st.stop()

    # --- LÓGICA DE ANOS DINÂMICOS (ESTILO PÍLULA) ---
    ...
    # Cores seguindo o padrão das outras abas
    estilo_design_anos = {
        2025: ("#E8F4F8", "#2E86C1"), 
        2026: ("#E8F8F5", "#28B463"), 
        2027: ("#FEF5E7", "#E67E22")
    ...
    for i, ano in enumerate(st.session_state.lista_anos_te):
        ...
        # CSS para botões estilo pílula arredondada
        st.markdown(f"""
            <style>
    ...
    with cols_anos_all[len(st.session_state.lista_anos_te)].popover("➕"):
        novo_ano_input = st.number_input("Novo ano:", 2024, 2100, st.session_state.lista_anos_te[-1] + 1)
    ...
    # --- SELEÇÃO DE SALA E ALUNO (USANDO BOTÕES COLORIDOS) ---
    # Busca salas da aba Turno Estendido via session_state (configurado no carregamento)
    salas_te = sorted(list(set(st.session_state.get("alunos_te_dict", {}).values())))
    
    if salas_te:
        if "sel_te" not in st.session_state or st.session_state.sel_te not in salas_te:
        ...
        # Puxa o histórico do aluno na aba Turno Estendido
        ...
        st.markdown("""<style>
            ...
            .caixa-trilha-te { padding: 6px 4px; border-radius: 12px; text-align: center; font-size: 10px; font-weight: bold; min-width: 105px; height: 50px; display: flex; align-items: center; justify-content: center; line-height: 1.1; box-shadow: 1px 1px 3px rgba(0,0,0,0.05); }
        ...
        for i, n_t in enumerate(NIVEIS_ALF):
            ...
            ht += f'<div class="caixa-trilha-te" style="background-color:{cor_bg}; color:{cor_txt}; border:{borda}; opacity:{opacidade};">{n_t.split(". ")[1]}</div>'
            if i < len(NIVEIS_ALF)-1: ht += '<span style="color:#D5DBDB; font-weight:bold; margin: 0 2px;">→</span>'
        st.markdown(ht + '</div>', unsafe_allow_html=True)

        # --- FORMULÁRIO DE REGISTRO ---
        try:
            ultimo_val = diag["Diagnóstico"] if diag is not None else ""
            idx_inicial = NIVEIS_ALF.index(ultimo_val) if ultimo_val in NIVEIS_ALF else 0
        except:
            idx_inicial = 0
            
        nV = st.selectbox("Novo Nível de Diagnóstico:", NIVEIS_ALF, index=idx_inicial)

        with st.form("f_te_registro"):
            ...
            st.write(f"**Evidências para o nível {nV}:**")
            e_cols = st.columns(3)
            s_ev = [ev for i, ev in enumerate(evidencias_atuais) if e_cols[i % 3].checkbox(ev, key=f"chk_te_{i}")]
            
            obs = st.text_area("Observações Adicionais:")
            
            if st.form_submit_button("🚀 Salvar Avaliação no Google Sheets"):
                # Mapeamento para gravar na planilha com nomes claros
                ...
                if sucesso:
                    st.success(f"Dados de {al} salvos com sucesso!")
                    st.cache_data.clear() # Limpa cache para ler o dado novo na trilha
                    st.rerun()
    else:
        st.warning("Nenhuma configuração de aluno/sala encontrada para o Turno Estendido.")
