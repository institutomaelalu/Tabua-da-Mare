...
# 1. Configuração e Estilo (CSS com Identidade Visual nas Caixas)
...
st.markdown(f"""
    ...
    /* --- CORES DA IDENTIDADE NAS CAIXAS DE SELEÇÃO --- */
    /* Estiliza o fundo das caixas de selectbox e inputs */
    div[data-baseweb="select"] > div, div[data-testid="stTextInput"] div[data-baseweb="base-input"] {{
        background-color: {C_AZUL}22 !important;
        border: 1px solid {C_AZUL}44 !important;
        border-radius: 8px !important;
    }}
    
    /* Alternância de cores automática para inputs consecutivos */
    ...
    div[data-testid="stVerticalBlock"] > div:nth-child(3n) div[data-baseweb="select"] > div {{
...
# 2. Configurações e Funções de Dados
...
for key in ['sel_mat', 'sel_pad', 'sel_aval']:
    if key not in st.session_state: st.session_state[key] = "SALA ROSA"
...
# --- DEFINIÇÃO DO MENU (ESSENCIAL PARA EVITAR NAMEERROR) ---
...
# --- ABA: APADRINHAMENTO ---
    ...
    with st.expander("✨ Vincular Novo Padrinho", expanded=False):
        ...
        if not sem_pad.empty:
            ...
            al_vinc = c1.selectbox("Aluno", sorted(sem_pad["ALUNO"].unique()))
...
# --- ABA: LANÇAR AVALIAÇÃO (Ajustado para Semestres) ---
    ...
    f1, f2 = st.columns(2)
    f_tn = f1.selectbox("Turno", ["Todos", "A", "B"], key="aval_tn")
    f_cm = f2.selectbox("Comunidade", ["Todas"] + sorted(list(df_geral["COMUNIDADE"].unique())), key="aval_cm")
    ...
    with st.form("aval"):
        if not df_f.empty:
            ...
            # Alterado de Trimestre para Semestre conforme pedido
            ...
            if st.form_submit_button("Salvar Avaliação"):
                ...
                st.success(f"Avaliação de {al} salva!")
        else: st.warning("Nenhum aluno encontrado para os filtros atuais.")
...
# --- ABA: EVOLUÇÃO ---
    ...
    if not afilhas.empty:
        ...
        else: st.info("Ainda não há avaliações lançadas para este afilhado.")
...
# --- ABA: TÁBUA DA MARÉ ---
    ...
    if not df_av.empty:
        ...
        df_al = df_av[df_av["Aluno"] == al_s]
        if not df_al.empty:
            tri = st.selectbox("Período", df_al["Periodo"].unique(), key="tri_tabua")
            row = df_al[df_al["Periodo"] == tri].iloc[0]
            fig = go.Figure(go.Scatter(x=CATEGORIAS, y=[float(row[c]) for c in CATEGORIAS], mode='lines+markers+text', text=[str(int(row[c])) for c in CATEGORIAS], fill='tozeroy', line=dict(color=C_VERDE, width=4, shape='spline')))
