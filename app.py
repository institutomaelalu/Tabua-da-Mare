...
if menu == "📝 Matrículas":
    ...
    if st.session_state.perfil == "admin":
        lista_p = sorted(df_g["PADRINHO/MADRINHA"].unique())
        padrinho_alvo = st.selectbox("Simular visão do Padrinho:", [p for p in lista_p if p and p != "0"])
    ...
    if not afilhas.empty:
        st.write(f"Olá **{padrinho_alvo}**, aqui está a evolução das crianças que você apadrinha:")
        ...
        if not df_al.empty:
            ...
            fig.update_layout(yaxis=dict(range=[0, 5.5]), height=350)
            st.plotly_chart(fig, use_container_width=True)
        else: st.info("Avaliações não lançadas para este aluno.")
