...
# --- SEÇÃO DETALHES AJUSTADA (NOME LIMPO E MARÉ 100%) ---
                ...
                with col_card:
                    ...
                    <div style="border:1px solid #ddd; padding:20px; border-radius:15px; background:#f9f9f9; color:black;">
                        <h4 style="margin-top:0; line-height:1.2;">{al_sel}</h4>
                        ...
                        <small style="color:#555;">{dados_h.iloc[-1]['Evidencias'] or 'Nenhum registro'}</small>
                        <p style="margin-top:15px; margin-bottom:5px;"><b>Observações:</b></p>
                        <i style="color:#555; font-size:13px;">{dados_h.iloc[-1]['Obs'] or 'Sem observações.'}</i>
                ...
                with col_visual:
                    ...
                    <style>
                        ...
                        /* Recipiente com fundo cinza claro */
                        .vasilha-ondulada {{ 
                        ...
                        /* Água preenchendo 100% da largura com onda ajustada */
                        .agua-preenchida-total {{
                            position: absolute; bottom: 0; left: 0; width: 100%; height: {altura};
                            background: #5DADE2;
                            /* O path da água agora encosta exatamente nas bordas 0 e 240 */
                            clip-path: path('M 0 10 Q 30 0 60 10 T 120 10 T 180 10 T 240 10 L 240 150 L 0 150 Z');
                            opacity: 1;
                        }}
                        .trilha-box {{ margin-top: 20px; font-size: 13px; color: black; border-top: 1px solid #eee; padding-top: 10px; }}
                        .trilha-item {{ padding: 6px 0; border-bottom: 1px dashed #eee; display: flex; justify-content: space-between; }}
                    </small>
                    </style>
                    <div class="container-mare">
                        <div class="vasilha-ondulada">
                            <div class="agua-preenchida-total"></div>
                        </div>
                    </div>
                    <div class="trilha-box">
                        <b style="color:#2E86C1;">📍 Trilha de Evolução:</b>
                        {"".join([f'<div class="trilha-item"><span>{row["Avaliacao"]}</span><b style="color:#555;">{row["Nivel"].split(". ")[1]}</b></div>' for _, row in dados_h.iterrows()])}
                    </div>
                    """, unsafe_allow_html=True)
